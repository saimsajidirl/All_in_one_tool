...
def convert_ppt_to_pdf(ppt_file):
    ...
    try:
        ...
        try:
            presentation = powerpoint.Presentations.Open(temp_ppt_path)
            presentation.SaveAs(temp_pdf_path, 32)  # 32 represents PDF format
            presentation.Close()
        finally:
            powerpoint.Quit()
...
def main():
    ...
    menu = st.sidebar.radio("Select Conversion",
                            ["PNG to ICO", "PDF to Word", "Merge PDFs",
                             "Images to PDF", "PPT to PDF", "JPG to PNG"])
    ...
    if menu == "PNG to ICO":
        ...
        if png_file is not None:
            st.image(png_file, caption="Uploaded PNG", use_container_width=True)
        ...
        if jpg_file is not None:
            st.image(jpg_file, caption="Uploaded JPG", use_container_width=True)
